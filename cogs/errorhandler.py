...
class ErrorHandler(commands.Cog):
    ...
    async def on_command_error(self, ctx: commands.Context, error):
        ...
        if classname in self._error_messages.keys():
            ...
            await ctx.send(
                ":rotating_light: An error occured while trying to execute that command"
            )
            ...
            embed.add_field(
                ...
                value=ctx.message.content[:1021]
            ...
            embed.add_field(
                ...
                else f"#{ctx.channel.name} (`{ctx.channel.id}`)",
            )
            embed.add_field(
                name="Sender", value=f"{str(ctx.author)} (`{ctx.author.id}`)"
            )

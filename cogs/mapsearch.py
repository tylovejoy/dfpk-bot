...
class MapSearch(commands.Cog, name="Map Search"):
    ...
    async def pages(self, ctx, contents, total_pages, map_name):
        ...
        def check(reaction, user):
            return user == ctx.author and str(reaction.emoji) in [
                constants.LEFT_REACTION_EMOJI, constants.RIGHT_REACTION_EMOJI]
        ...
        while True:
            try:
                ...
                if str(
                        reaction.emoji) == constants.RIGHT_REACTION_EMOJI and cur_page != total_pages:
                    cur_page += 1
                    await message.edit(
                        content=f"{map_name}\n```Page {cur_page}/{total_pages}:\n{contents[cur_page - 1]}```")
                    await message.remove_reaction(reaction, user)
                        ...
                        reaction.emoji) == constants.LEFT_REACTION_EMOJI and cur_page > 1:
                    cur_page -= 1
                    await message.edit(
                        content=f"{map_name}\n```Page {cur_page}/{total_pages}:\n{contents[cur_page - 1]}```")
                    await message.remove_reaction(reaction, user)
    ...
    async def creator(self, ctx, creator):
        ...
        if row:
            if ceil(row / constants.PT_PAGE_SIZE) != 1:
                ...
                await self.pages(ctx, split_pt, ceil(row / constants.PT_PAGE_SIZE),
                                 creator.capitalize())
            else:
    ...
    async def megamap(self, ctx):
        ...
        async for entry in MapData.find({"type": "MEGAMAP"}):
            pt.add_row(
                [entry.code, constants.PRETTY_NAMES[entry.map_name], fill(entry.desc, constants.DESC_MAX_LENGTH),
                 fill(entry.creator, constants.CREATOR_MAX_LENGTH)])
            row += 1
        if row:
            if ceil(row / constants.PT_PAGE_SIZE) != 1:
    ...
    async def ayutthaya(self, ctx, map_type=""):
        ...
        async for entry in MapData.find(
                ...
                if map_type else {"map_name": f"{''.join(map_name.split()).lower()}"}):
            pt.add_row(
                [entry.code, fill(' '.join(entry.type), constants.TYPE_MAX_LENGTH),
                 fill(entry.desc, constants.DESC_MAX_LENGTH),
                 fill(entry.creator, constants.CREATOR_MAX_LENGTH)])
            row += 1
        if row:

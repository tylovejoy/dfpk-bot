...
class SubmitMap(commands.Cog, name="Map submission/deletion"):
    ...
    async def submitmap(self, ctx, map_code, map_name, map_type, creator, *, desc=""):
        if '$' in map_code or '$' in map_name or '$' in map_type or '$' in creator or '$' in desc:
            map_code.replace('$', '')
            map_name.replace('$', '')
            map_type.replace('$', '')
            creator.replace('$', '')
    ...
    async def deletemap(self, ctx, map_code):
        ...
        if await MapData.count_documents({"_id": map_code}) == 1:
            ...
            if search.posted_by == ctx.author.id or (True if any(
                ...
                pt = prettytable.PrettyTable(
                    field_names=["Map Code", "Map Type", "Map Name", "Description",
                                 "Creator"]
                )
                pt.add_row([
                    search.code,
    ...
    async def editdesc(self, ctx, map_code, *, desc):
        ...
        if await MapData.count_documents({"_id": map_code}) == 1:
            ...
            if search.posted_by == ctx.author.id or (True if any(
                ...
                pt.add_row(
                    [search.code, fill(' '.join(search.type), constants.TYPE_MAX_LENGTH),
                     constants.PRETTY_NAMES[search.map_name],
                     fill(search.desc, constants.DESC_MAX_LENGTH),
                     fill(search.creator, constants.CREATOR_MAX_LENGTH)])
